...
The log-likelihood of each genotype is obtained in this fashion. The log-likelihood for each of the 10 genotypes ($P_{log}(R|\mathbb{G} = \{A_1,A_2\})$) is all we need for the purpose of calculating a reference quality score. (Note: ANGSD does not consider mapping quality when calculating genotype likelihoods). We also scale the genotype log-likelihoods by subtracting the largest likelihood from each score:
